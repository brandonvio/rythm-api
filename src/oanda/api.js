        ...
        throttledRequest(
            ...
            (err, res, body) => {
                if (res.statusCode !== 200) {
                    processApiError("getAccounts", null, res.statusCode, res.statusMessage, response);
                } else {
        ...
        throttledRequest(
            ...
            (err, res, data) => {
                ...
                if (!err && !body.errorMessage && !body.code) {
                    ...
                    processApiError("getCandles", err || body.errorMessage, body.code, body.message, response);
                }
            }
        );
    }
...
function closeOrder(req, response) {
    if (!req.body) {
        ...
        const api = config.getUrl(req.body.environment, "api");
        const url = `${api}/v3/accounts/${req.body.accountId}/orders` + `/${req.body.id}/cancel`;
...
function closeTrade(req, response) {
    if (!req.body) {
        ...
        const api = config.getUrl(req.body.environment, "api");
        const url = `${api}/v3/accounts/${req.body.accountId}/trades` + `/${req.body.id}/close`;
...
function processApi(apiName, err, data, response, property) {
    ...
    try {
        ...
        if (!err && !body.errorMessage && !body.errorCode) {
            ...
            processApiError(apiName, err || body.errorMessage, body.errorCode, body.errorMessage, response);
        }
    } catch (e) {
